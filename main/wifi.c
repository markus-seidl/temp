                                ...
                                int32_t event_id, void* event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        ESP_LOGI(TAG, "Connected to AP" IPSTR, IP2STR(&event->ip_info.ip));
        s_retry_num = 0;
...
uint8_t wifi_init_sta(void)
{
    ...
    esp_netif_t* wif = esp_netif_create_default_wifi_sta();

    // disable DHCP-Client and set static IP
    ESP_ERROR_CHECK(esp_netif_dhcpc_stop(wif));

    esp_netif_ip_info_t ip_info;
    IP4_ADDR(&ip_info.ip, 192, 168, 20, 120);
   	IP4_ADDR(&ip_info.gw, 192, 168, 20, 1);
   	IP4_ADDR(&ip_info.netmask, 255, 255, 255, 0);

    ESP_ERROR_CHECK(esp_netif_set_ip_info(wif, &ip_info));
